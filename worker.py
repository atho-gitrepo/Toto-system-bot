...
class AutomationWorker:
    ...
    def auto_generate_numbers(self):
        ...
        try:
            logger.info("Checking if generation needed...")
            ...
            # Check if we should generate (2 days before draw)
            days_until = next_draw.get('days_until', 999)
            
            if days_until <= 2 and days_until >= 0:
                logger.info(f"Generating numbers for draw {draw_no} (in {days_until} days)")
...
If auto-fetch fails, use: /declare winning numbers</i>"""
                ...
                logger.info(f"Not time to generate (draw in {days_until} days)")
                
        except Exception as e:
            logger.error(f"Auto-generation error: {e}", exc_info=True)
    
    def auto_check_results(self):
        """Check results for completed draws"""
        try:
            ...
            draw_result = self.result_service.get_draw_result(draw_no)
...
(Replace with actual winning numbers from Singapore Pools website)"""
                self.bot.telegram.send_message(message)
    ...
    def _process_results(self, draw_no: str, generation: dict, draw_result: dict):
        ...
        result_data = {
            ...
            'source': draw_result.get('source', 'auto_fetch')
        }
...
Winning Numbers: {draw_result['winning_numbers']} + {draw_result.get('additional_number', 0)}
━━━━━━━━━━━━━━━━━━
...
<i>Numbers for next draw will be generated automatically!</i>
...
Check /roi for updated statistics."""
    ...
    def health_check(self):
        ...
        try:
            ...
            logger.info(f"💚 Health Check - Games: {stats.get('games_played', 0)}, "
                       ...
                       f"Next Draw: #{next_draw['draw_no']} on {next_draw['draw_date']}")
        except Exception as e:
    ...
    def setup_schedules(self):
        ...
        schedule.every().day.at("22:00").do(self.auto_check_results)
        
        # Health check
        schedule.every().day.at("09:00").do(self.health_check)
        
        logger.info("✅ Schedules configured:")
        logger.info("  - Generation: 8 AM, 12 PM, 4 PM")
        logger.info("  - Result checking: Every 2 hours + 8:30 PM, 9 PM, 10 PM")
        logger.info("  - Health check: Daily at 9 AM")
    
    def run(self):
        """Run worker"""
        logger.info("=" * 60)
        logger.info("TOTO SYSTEM 8 - HYBRID AUTOMATION WORKER")
        logger.info("Auto-fetch + Manual fallback enabled")
        logger.info("=" * 60)
